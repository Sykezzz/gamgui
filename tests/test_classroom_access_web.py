    ...
    def __init__(self):
        ...
        self.users = {
            "teacher@example.org": GAMUser(
                "teacher@example.org", org_unit_path="/Staff/Teachers"
            ),
            "admin@example.org": GAMUser(
                "admin@example.org", org_unit_path="/Staff/Administrators"
            ),
        }
...
def _client(tmp_path):
    ...
    async def directory_groups(query="", **_kwargs):
        items = [
            group
            for group in connector.groups
            if query.casefold() in f"{group.email} {group.name}".casefold()
        ]
        return Page(
            items=items,
            next_cursor=None,
    ...
    async def directory_org_units(query="", **_kwargs):
        items = [
            path
            for path in ("/Staff/Administrators", "/Staff/Teachers")
    ...
    state.directory_org_units = directory_org_units
    app = FastAPI()
...
def test_access_page_is_discoverable_and_has_safe_defaults(tmp_path):
    ...
    assert "Search groups by name or email" in response.text
    assert "Search organizational-unit paths" in response.text
    assert 'role="combobox"' in response.text


def test_directory_source_pickers_are_live_and_bounded(tmp_path):
    client, _state = _client(tmp_path)

    groups = client.get(
        "/classroom/access/options",
        params={"kind": "groups", "q": "admin"},
    )
    org_units = client.get(
        "/classroom/access/options",
        params={"kind": "org_units", "q": "teacher"},
    )

    assert groups.status_code == 200
    assert 'role="option"' in groups.text
    assert "administrators@example.org" in groups.text
    assert "staff@example.org" not in groups.text
    assert org_units.status_code == 200
    assert "/Staff/Teachers" in org_units.text
    assert "/Staff/Administrators" not in org_units.text


def test_group_and_ou_sources_preview_as_one_exact_union(tmp_path):
    ...
    preview = client.post(
        ...
        data={
            ...
            "source_org_units": "/Staff/Administrators",
            "schedule_hour": "2",
            "schedule_minute": "0",
        },
    )
    ...
    assert policy.source_org_units == ("/Staff/Administrators",)
    assert plan.adds == ("admin@example.org", "staff@example.org")
    assert "1 group" in preview.text
    assert "1 OU" in preview.text
