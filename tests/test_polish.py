...
def test_makefile_setup_selects_the_interpreter_via_a_variable():
    """The locked uv sync must use an overridable, verified Python 3.10+ interpreter."""
    ...
    for line in setup.splitlines():
        assert "--python python3" not in line, f"setup hardcodes the system interpreter: {line.strip()}"
    assert "$(PYTHON)" in setup and "$(PYTHON_CANDIDATES)" in setup
    assert 'uv $(UV_VERSION)' in setup
    assert 'sync --frozen --python "$$py"' in setup
