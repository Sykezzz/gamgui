...
from gamgui.core.gam.models import GAMUser, GroupMember
...
def test_board_member_email_cannot_break_out_of_its_attribute():
    page = Page([GroupMember(email=HOSTILE)], None, 1, None, False)
    html = _render("_board_members.html", group="staff@example.com", page=page)
...
def test_board_group_name_cannot_break_out_of_hx_vals():
    ...
    page = Page([GroupMember(email="bob@example.com")], None, 1, None, False)
    html = _render("_board_members.html", group=HOSTILE, page=page)
...
def test_people_pool_user_cannot_break_out_of_its_attribute():
    page = Page([GAMUser(primary_email=HOSTILE, given_name=HOSTILE, family_name="")], None, 1, None, False)
    html = _render(
        "_people_search_results.html",
        page=page,
    )
...
def test_drag_and_drop_still_reads_the_email_from_the_card():
    ...
    people_page = Page([GAMUser(primary_email="a@example.com")], None, 1, None, False)
    member_page = Page([GroupMember(email="a@example.com")], None, 1, None, False)
    pool = _render("_people_search_results.html", page=people_page)
    board = _render("_board_members.html", group="staff@example.com", page=member_page)

    assert "gqDrag(event, this.dataset.email, 'add')" in pool
    assert "gqDrag(event, this.dataset.email, 'remove')" in board


def test_board_renders_over_http(client):  # noqa: F811
    r = client.get("/groups/members", params={"group": "team@example.com"})
    assert r.status_code == 200
    assert "gqDrag(event, this.dataset.email, 'remove')" in r.text
